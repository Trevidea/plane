"use client";
import { Check, Mic, Play, RefreshCw, Square, Trash2 } from "lucide-react";
...
export const VoiceNarrationPanel = ({
  ...
  const showInspector = clip && audio && (state.stage === "idle" || state.stage === "review");
  return (
    <section
    ...
    >
      ...
      {showInspector && !conflict ? (
        <>
          {draft ? (
            <p role="status" className="flex items-center gap-2 font-medium text-custom-text-100">
              <Check className="size-4 text-green-500" />
              Narration recorded · {(clip.endTime - clip.startTime).toFixed(2)} sec
            </p>
          ) : null}
          {state.warning ? (
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {draft ? "Re-record" : "Replace"}
            </Button>
            {draft ? (
              <Button size="sm" onClick={onAccept} prependIcon={<Check />}>
                Done
              </Button>
            ) : null}
          </div>
                  ...
                  <Input
                    ...
                    onChange={(event) => {
                      if (event.target.value !== "")
                        onChange(moveNarration(clip, Number(event.target.value), duration));
                    }}
                    className="w-full"
                  />
                </label>
