...
class TestGetDocumentThumbnailHint:
    ...
    def test_uses_video_icon_for_default_plane_coach_json_poster(self):
        assert (
            get_document_thumbnail_hint(
                "json",
                {"source": "plane-coach", "poster_url": "/coach/defualt.jpg"},
            )
            == "attachment/video-icon.png"
        )

    def test_uses_custom_poster_hint_before_plane_coach_json_fallback(self):
        assert (
            get_document_thumbnail_hint(
                "json",
                {"source": "plane-coach", "poster_url": "/coach/custom-poster.jpg"},
            )
            == "/coach/custom-poster.jpg"
        )
...
class TestGetDocumentIconSource:
    def test_resolves_plane_coach_public_video_icon_hint(self):
        icon_source = get_document_icon_source("json", "attachment/video-icon.png")

        assert icon_source is not None
        assert icon_source.name == "video-icon.png"
